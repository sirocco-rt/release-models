...
def make_figure(quasar_path="{}/Demos/quasar/".format(util.g_DataDir),  **savefig_kwargs):
    ...
    for ii, i in enumerate((10, 50, 55)):
        x, y = spectrum_file["Lambda"], util.smooth(
            spectrum_file[f"A{i}P0.50"])
        ax1.plot(
            x,
            y,
            label=r"$i = {i}^\circ$",
    ...
    util.save_paper_figure("quasar_demo_model.pdf",
