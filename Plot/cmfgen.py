...
def make_figure(path="{}/Tests/cmfgen/".format(util.g_DataDir), root="a3_hhe", **savefig_kwargs):
    ...
    for i, ax in enumerate(axes):
        ax.plot(star['Lambda'], norm*star_nufnu,
                label=util.CODE_NAME, zorder=py_zorder, c="C0", lw=2.5)
        ax.plot(cmfgen['Wave'], norm*cmfgen['smooth'],
                label='CMFGen', c="k", lw=3, alpha=0.8)
    ...
    util.save_paper_figure("cmfgen_comparison.pdf", fig=fig, **savefig_kwargs)
