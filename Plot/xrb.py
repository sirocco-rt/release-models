...
def make_figure(quasar_path="{}/Demos/xrb/".format(util.g_DataDir), **savefig_kwargs):
    ...
    for ii, i in enumerate((70, 80)):
        x, y = HEV * spectrum_file["Freq."] / 1e3, 1e22 * \
            spectrum_file["Freq."]*conv*spectrum_file[f"A{i}P0.50"]
    ...
    ax1.set_xlabel(
        r"$E$ (keV)", fontsize=util.onepanel_labelsize, labelpad=-0.1)

    z, ion, w, e_line, id, offset = np.genfromtxt(
        StringIO(line_data), unpack=True)
    ax1.vlines(e_line, 6.1e-3, 4, color="k",
               lw=1, ls="--", zorder=1, alpha=0.6)
    ...
    for i in range(len(e_line)):
        ...
        if id[i] == 1:
            ...
            ax1.text(e_line[i], 1.2e-3, label, rotation=90,
                     fontsize=12, ha="center", va="bottom", bbox=bbox)
    ax1.text(6.8, 1.2e-3, r"Fe \textsc{xxv},\textsc{xxvi}",
             rotation=90, fontsize=12, ha="center", va="bottom", bbox=bbox)
    # ax1.vlines(e_line, 5e-3,3, ls="--", color="k", lw=1)

    # Add inset figure
    inset_ax = inset_axes(ax1, width="35%", height="30%",
    ...
    util.save_paper_figure("xrb_demo_model.pdf", fig=fig,
                           dpi=300, **savefig_kwargs)
