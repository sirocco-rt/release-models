...
def plot_one(ax=None, cloudy='cl_helium_PL.dat', python='py_he', element='He', imin=0, imax=0, ipmin=-8, ipmax=4, outname=''):
    ...
    while i < imax:
        ...
        ax.semilogy(xc['ip'], xc[colname[i]], '-', lw=3,
                    alpha=0.5, color=color_cycle[color_index])
        ax.semilogy(np.log10(xp['ip']), xp[colname[i]], "--",
                    color=color_cycle[color_index], label=xlabel)
        i += 1
        color_index += 1
...
def get_chi(ion, python=True, cl=None, python_dir="./"):
    if python:
        ...
        xx = (np.sum(states * ion_densities.T, axis=1)-1) / \
            (len(ion_densities)-1)
        ...
        xcip = xc[0]
        cxx = (np.sum(states * cion_densities.T, axis=1)-1) / \
            (len(cion_densities)-1)
        return (xcip,  cxx)


def make_figure(path="{}/Tests/cloudy/".format(util.g_DataDir), **savefig_kwargs):
    """
    ...
    for i, ion in enumerate(["H", "He", "C", "O", "Fe"]):
        ...
        if ion == "Fe":
            plot_one(ax, cloudy=cl[ion], python=py_fname,
                     element=ion, imin=19, imax=27, ipmin=0, ipmax=7)

        else:
            plot_one(ax, cloudy=cl[ion], python=py_fname,
                     element=ion, imin=0, imax=0, ipmin=-6, ipmax=4)
    ...
    util.save_paper_figure("cloudy_comparison.pdf", fig=fig, **savefig_kwargs)
