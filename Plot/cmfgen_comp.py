...
def make_figure(path="{}/Tests/cmfgen/".format(util.g_DataDir), root="cmfgen_try"):
    ...
    he_frac = Table.read("{}/{}.He.frac.txt".format(path, root), format='ascii')
    ...
    ax.plot(r2, he2, linestyle='-', c="C0", alpha=0.7, lw=4, label=r"He~\textsc{ii}, \textsc{cmfgen}")
    ax.plot(r3, he3, linestyle='-', c="C1", alpha=0.7, lw=4, label=r"He~\textsc{ii}, \textsc{cmfgen}")
    ax.plot(he_frac["r"]/1.4e12, he_frac["i02"], "-o", c="C0", label=r"He~\textsc{{iii}}, {}".format(util.CODE_NAME))
    ax.plot(he_frac["r"]/1.4e12, he_frac["i03"], "-o", c="C1", label=r"He~\textsc{{iii}}, {}".format(util.CODE_NAME))
    ax.loglog()
    ...
    util.save_paper_figure("cmfgen_ions.pdf", fig=fig)
