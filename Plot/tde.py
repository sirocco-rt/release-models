...
def make_figure(tde_path="{}/Demos/tde".format(util.g_DataDir),  **savefig_kwargs):
    ...
    optical_depth_spectrum = ascii.read(
        "{}/tde_standard.spec_tau".format(tde_path))
    ...
    for i in (10, 60, 75):
        x, y = util.get_flux_range(
            spectrum_file["Lambda"], util.smooth(
                spectrum_file[f"A{i}P0.50"]), 1000, 3000
        )
        ax1.plot(
            x,
            y,
    ...
    for i in (10, 60, 75):
        x, y = util.get_flux_range(
            spectrum_file["Lambda"], util.smooth(
                spectrum_file[f"A{i}P0.50"]), 4000, 8000
        )
        ax2.plot(
            x,
            y,
    ...
    ax2.text(0.05, 0.1, "Optical", transform=ax2.transAxes,
             fontsize=16, ha="left")
    # Reprocessing
    ax3 = plt.subplot(gs[:, 1])
    ax3_twin = ax3.twinx()
    # emitted vs. created
    ax3.plot(spectrum_file["Freq."], util.smooth(
        spectrum_file["Emitted"]), label="Emergent", lw=2.5)
    ax3.plot(spectrum_file["Freq."], util.smooth(
        spectrum_file["Created"]), label="Disc Input", lw=2.5)
    # optical depth
    ...
    ax3.legend()
    ax3_twin.set_ylabel(r"Continuum optical depth $\tau$",
                        fontsize=util.onepanel_labelsize)
    ax3.text(0.05, 0.1, "Global Reprocessing",
             transform=ax3.transAxes, fontsize=16, ha="left")
    # Clean up the figure
    fig.tight_layout(pad=0.05)
    util.save_paper_figure("tde_demo_model.pdf", fig=fig,
