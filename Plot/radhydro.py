...
def make_figure(cv_model="00001743.pluto", xdir="{}/Demos/rad-hydro/".format(util.g_DataDir),  **savefig_kwargs):
    '''
    ...
    data_interp["xx"], data_interp["yy"] = np.meshgrid(
        data_interp["x"], data_interp["y"])
    data_interp["rr"] = np.sqrt(data_interp["xx"]**2 + data_interp["yy"]**2)
    select_mask = (data_interp["rr"] < rmin)
    for key in d.keys():
        if key not in ["x", "y"]:
            data_interp[key] = np.array(interpolate_data(
                data, key, data_interp["xx"], data_interp["yy"]))
    ...
    ax[0].pcolormesh(x, y, np.log10(d["density"]*rho2nh), lw=0,
                     rasterized=True, shading="gouraud", cmap=cmap, vmin=6, vmax=14)
    v = np.sqrt(d["v_x"]**2 + d["v_y"]**2)
    # ax[0].contour(y,x,v/(np.sqrt(4.0)*1e6), levels=(1,10), colors="C3")
    mappable = ax[1].pcolormesh(x, y, np.log10(
        d["density"]*rho2nh), lw=0, rasterized=True, shading="gouraud", cmap=cmap, vmin=6, vmax=14.5)
    ...
    plt.subplots_adjust(left=0.05, right=0.89, top=0.96,
                        wspace=0.1, hspace=0.05, bottom=0.12)
    # print (util.get_aspect(ax[0]))
    util.save_paper_figure('rad-hydro2.pdf',  **savefig_kwargs)
