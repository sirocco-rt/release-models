...
def make_figure(cv_model="00001743.pluto", xdir="{}/Demos/rad-hydro/".format(util.g_DataDir)):
    ...
    fig, ax = plt.subplots(
        nrows=1, ncols=2, figsize=(10, 5))  # 1 rows, 2 columns
    ...
    ax[0].pcolormesh(x, y, np.log10(d["density"]), lw=0,
                     rasterized=True, shading="gouraud", cmap=cma.cosmic)
    ax[0].pcolormesh(-x, -y, np.log10(d["density"]), lw=0,
                     rasterized=True, shading="gouraud", cmap=cma.cosmic)
    ax[0].pcolormesh(-x, y, np.log10(d["density"]), lw=0,
                     rasterized=True, shading="gouraud", cmap=cma.cosmic)
    ...
    plt.pcolormesh(data_xrb["x"].reshape(shape), data_xrb["z"].reshape(shape), np.log10(
        data_xrb["rho"].reshape(shape2).T), shading="gouraud", cmap="jet")
